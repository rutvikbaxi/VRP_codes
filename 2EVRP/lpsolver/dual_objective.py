...
def dual_objective(parcel_list, depot_list):
    ...
    model+=pulp.lpSum(tdc[d][c]*x[r][c][d] for r in ROBOTS for c in CUSTOMERS for d in DEPOTS) + 1000000*pulp.lpSum(o[i] for i in DEPOTS)
    # model+=pulp.lpSum(o[i] for i in DEPOTS)
    ...
    total_dist=pulp.lpSum(tdc[d][c]*value(x[r][c][d]) for r in ROBOTS for c in CUSTOMERS for d in DEPOTS)
    print(total_dist, "using #dual optimization")
    print(lpSum(value(o[i]) for i in DEPOTS), "#hubs selected")
    return model, arcs
